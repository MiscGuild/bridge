...
export default {
	...
	run: async (bot, rank: string | undefined, playerName: string, type: "joined" | "left") => {
		...
		await bot.sendToDiscord("gc", `${type === "joined" ? Emojis.positiveGuildEvent : Emojis.negativeGuildEvent} **${rank ? `${rank} ` : ""}${escapeMarkdown(playerName)}** ${type} the guild!`, getRankColor(rank), true);
				...
				fetch(`https://api.hypixel.net/guild?key=${env.HYPIXEL_API_KEY}&name=${env.HYPIXEL_GUILD_NAME}`)
					...
					.then((data) => {
						...
						if (isFetchError(mojangProfile)) {
							...
							bot.mineflayer.once("message", async (message) => {
								const messageContent = message.toString();
								const logEntries = messageContent.split("\n");

								// Define the regex once (without the global flag)
								const regex =
								...
								// Gather all log entries that match the regex.
								const validEntries = logEntries.filter((entry) => regex.test(entry));

								// Check after processing all entries.
				...
				fetch(`https://api.hypixel.net/guild?key=${env.HYPIXEL_API_KEY}&name=${env.HYPIXEL_GUILD_NAME}`)
					...
					.then((data) => {
						...
						if (isFetchError(mojangProfile)) {
							...
							bot.sendToDiscord("oc", `Player **${escapeMarkdown(playerName)}** left the guild! Their join date was ||${formattedLeaveDate}||. They stayed in the guild for **${diffDays}** days. `);
							return resolve();
						}
					});
			});
