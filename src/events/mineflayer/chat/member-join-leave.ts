 ...
 */
async function sendGuildEventMessage(bot: any, rank: string | undefined, playerName: string, type: "joined" | "left"): Promise<void> {
	const message = `${type === "joined" ? Emojis.positiveGuildEvent : Emojis.negativeGuildEvent} **${rank ? `${rank} ` : ""}${escapeMarkdown(playerName)}** ${type} the guild!`;
...
async function processJoinEvent(bot: any, playerName: string, mojangProfile: any): Promise<void> {
	const response = await fetch(`https://api.hypixel.net/guild?key=${env.HYPIXEL_API_KEY}&name=${env.HYPIXEL_GUILD_NAME}`);
	...
	if (isFetchError(mojangProfile)) {
		console.log(`[DEBUG] ${playerName} joined the guild, but failed to get guild data.`);
		bot.executeCommand(`/oc Failed to get guild data of recently joined member ${playerName}.`);
		throw new Error("Playername was invalid. Probably an error with the regex or the username was unusual");
	}
	...
	await new Promise<void>((resolve, reject) => {
		bot.mineflayer.once("message", async (message: any) => {
			...
			const regex = /(([A-Za-z]{3}\s[0-9]{1,2}\s[0-9]{4}) (([0-9]{2}):([0-9]{2})) ((EDT|EST))): ([A-Za-z0-9-_]{2,27}) (joined|left|invited|kicked|muted|unmuted|set rank of|set MOTD|set guild tag|set guild tagcolor|set Discord|turned the chat throttle on|turned the chat throttle off)( ([A-Za-z0-9-_]{2,27})?)?( for | to |: )?([ A-Za-z0-9\-!_\\s]+)?/;
			const validEntries = logEntries.filter((entry: string) => regex.test(entry));


			if (validEntries.length >= 1) {
				if (validEntries[1]?.includes("invited")) {
					bot.sendToDiscord("oc", `Player **${escapeMarkdown(playerName)}** joined the guild! Their join date is ||${joinDatetoSend}||.\n\nThey were invited by **${validEntries[1].split(" ")[5]}**.`);
				}
				else
				{
				...
				console.log(`[DEBUG] ${playerName} joined the guild, but failed to get invite data.`);
				reject("Invite data not found, this is an issue with /g log and the associated regex");
			}
		});
	});
}
...
async function processLeaveEvent(bot: any, playerName: string, mojangProfile: any): Promise<void> {
	...
	const formattedJoinDate = new Date(joinDate).toLocaleString("en-US", { timeZone: "Europe/Amsterdam" });

	bot.sendToDiscord(
		"oc",
		`Player **${escapeMarkdown(playerName)}** left the guild! Their join date was ||${formattedJoinDate}||. They stayed in the guild for **${diffDays}** days.`
	);
}
...
export default {
	...
	runOnce: false,
	run: async (bot: any, rank: string | undefined, playerName: string, type: "joined" | "left") => {
		if (type === "joined") {
