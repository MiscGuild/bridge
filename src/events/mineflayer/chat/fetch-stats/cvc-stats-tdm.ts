					...
					fetch(`https://api.hypixel.net/player?key=${process.env.HYPIXEL_API_KEY}&name=${_playerName}`)
						...
						.then((data) => {
							...
							if (!data.player.stats || !data.player.stats.MCGO || !data.player.achievements) {
								console.log(`[DEBUG] ${_playerName} is checking the stats of ${_playerName}, but incomplete data was received. | ${getRandomHexColor()}`);
							...
							bot.executeCommand(`/gc [CVC-TDM] IGN: ${_playerName} | KILLS: ${_kills} | WINS: ${_wins} | KDR: ${_kdr.toFixed(2)} | HEADSHOT KILLS: ${_headshots} | ${getRandomHexColor()}`);
				...
				return new Promise((resolve, reject) => {
					fetch(`https://api.hypixel.net/player?key=${process.env.HYPIXEL_API_KEY}&name=${_target}`)
						...
						.then((data) => {
							...
							console.log(`[DEBUG-TDM] ${_playerName} is checking the stats of ${_target} and succeeded`);

							bot.executeCommand(`/gc [CVC-TDM] IGN: ${_target} | KILLS: ${_kills} | WINS: ${_wins} | KDR: ${_kdr.toFixed(2)} | HEADSHOT KILLS: ${_headshots} | BOMBS PLANTED: ${_bombs} | ${getRandomHexColor()}`);
