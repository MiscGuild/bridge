...
function buildStatsMessage(playerName: string, achievements: Achievements, stats: Duels): string {
    const kills = stats?.sumo_duel_kills ?? 0;
    const deaths = stats?.sumo_duel_deaths ?? 0;

    const wins = stats?.sumo_duel_wins ?? 0;
    const losses = stats?.sumo_duel_losses ?? 0;

    const wlr = (losses === 0 ? wins : wins / losses).toFixed(2);
    const kdr = (deaths === 0 ? kills : kills / deaths).toFixed(2);

    return `/gc [Sumo Duels] IGN: ${playerName} | WINS: ${wins} | KILLS: ${kills} | KDR: ${kdr} | WLR: ${wlr} | ${getRandomHexColor()}`;
}
